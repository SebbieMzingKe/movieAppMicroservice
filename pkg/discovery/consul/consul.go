...
func (r *Registry) Register(ctx context.Context, instID string, sName string, hostPort string) error {
	...
	if len(parts) != 2 {
		return errors.New("hostPort must be in for of <host>:<port>, example: localhost:8081")
	}
...
func (r *Registry) Deregister(ctx context.Context, instID string, _ string) error {
...
func (r *Registry) ServiceAddresses(ctx context.Context, sName string) ([]string, error) {
	entries, _, err := r.client.Health().Service(sName, "", true, nil)
